...
overview_metrics = ['profit-list', 'rate-of-return', 'aggregate-profit-by-day']
...
def update_metrics(ts1, ts2, profit_list, rate_of_return):
    if profit_list is None:
        return 'Confirm Data'
    total_profit = sum(profit_list)
    profit_factor = rate_of_return + 1

    return round(total_profit, 2), round(rate_of_return, 2), round(profit_factor, 2)
